...
This report is focus on the Exploratory Analysis results and includes:

  - Understanding the data,
  - Getting and Cleaning the Data
  - Exploratory Data Analysis
   
I have ommitted some of the code to make this report concise. The complete code is avalable [here]().
...
Before tokenizing the data, we perform the following cleaning steps:
  ...
  - Remove \\"
  - Remove punctuation without removing strings such as: in "St. Charles" expression
...
```{r GettingProfanityDBCleaningDataset, eval = TRUE, echo = FALSE}
require(httr)
require(XML)
...
#### 4.1 Frequency of words, pairs of words and triple of words
...
#### 4.2 Distribution of words, pair of words, triple of words
...
```{r figures, eval = TRUE, echo  = FALSE, message = FALSE, warnings = FALSE, fig.width = 10, fig.height = 4, fig.align = 'center'}
require(ggplot2) 
...
# Top 15 bi-grams
g1 <- plotFrequency (df_bitext[df_bitext$type =="blog",][1:n,], "Frequency of pairs of words in Blog text",
                     "Top 15 pairs of words", col = 2)
g2 <- plotFrequency (df_bitext[df_bitext$type =="news",][1:n,], "Frequency of pairs of words in News text",
                     "Top 15 pairs of words", col = 3)
g3 <- plotFrequency (df_bitext[df_bitext$type =="twitter",][1:n,], "Frequency of pair of words in Twitter text",
                     "Top 15 pairs of words", col = 4)
grid.arrange(g1, g2, g3, ncol=3)

# Top 15 tri-grams
g1 <- plotFrequency (df_tritext[df_tritext$type =="blog",][1:n,], "Frequency of triple words in Blog text",
                     "Top 15 triple of words", col = 2)
g2 <- plotFrequency (df_tritext[df_tritext$type =="news",][1:n,], "Frequency of triple words in News text",
                     "Top 15 triple of words", col = 3)
g3 <- plotFrequency (df_tritext[df_tritext$type =="twitter",][1:n,], "Frequency of triple words in Twitter text",
                     "Top 15 triple of words", col = 4)
grid.arrange(g1, g2, g3, ncol=3)
```
...
v_coverage <- c(cutoff(df_displayp[df_displayp$type == "blog",])[2] / nrow(df_unitext[df_unitext$type =="blog",]),
               cutoff(df_displayp[df_displayp$type == "news",])[2] / nrow(df_unitext[df_unitext$type =="news",]),
               cutoff(df_displayp[df_displayp$type == "twitter",])[2] / nrow(df_unitext[df_unitext$type =="twitter",]),
               cutoff(df_displayp[df_displayp$type == "blog",])[3] / nrow(df_unitext[df_unitext$type =="blog",]),
               cutoff(df_displayp[df_displayp$type == "news",])[3] / nrow(df_unitext[df_unitext$type =="news",]),
               cutoff(df_displayp[df_displayp$type == "twitter",])[3] / nrow(df_unitext[df_unitext$type =="twitter",]),
               cutoff(df_displayp[df_displayp$type == "blog",])[4] / nrow(df_unitext[df_unitext$type =="blog",]),
               cutoff(df_displayp[df_displayp$type == "news",])[4] / nrow(df_unitext[df_unitext$type =="news",]),
               cutoff(df_displayp[df_displayp$type == "twitter",])[4] / nrow(df_unitext[df_unitext$type =="twitter",]))
...
```{r ForeignLanguage, eval = FALSE, echo = TRUE}
...
print(head(df_tritext[occurrences, c(1,2,5)], 5), row.names=FALSE)
```
...
print(head(df_quadritext[occurrences, c(1,2,5)], 5), row.names=FALSE)
```

For efficiency, I store the n-grams models in a [Markov chain](https://en.wikipedia.org/wiki/Markov_chain). 
...
A 4-gram can be seen as a chain of 4 states, where what happens on the 4th state depends only on the current state of the system. My Markov chain uses a 4 dimensions transition matrix. This matrix models the frequency of words following 1, 2, or 3 given words. 

To compute the probablility of each n-gram in the transition matrix, I use a [Katz back-off](https://en.wikipedia.org/wiki/Katz%27s_back-off_model) model. 
...
To compute the model accuracy, I take each sentence from the test set, get 3 consecutive words and predict the 4th one with 3 likely results. I consider the prediction accurate of one of the three proposed words matches the 4th word in the sentence.
